...
void domain::initTriangulateAdvance(){
	unsigned long int startPointIndex = 0;
	unsigned long int extensionPointIndex = pointNumMax+4;

	//create an array of points = init points + extention points (not inluding 4 corner points)
	unsigned int initExtendPointArrSize = initPointArrSize + (extensionPointArrSize - 4);
	point *initExtendPointArr = new point[initExtendPointArrSize];
	unsigned int pointIndex = 0;
	
	//triangulate extension points (not including 4 corner points because they are in 2 big triangles ABC & ACD)
	//4 means  not include 4 corners
	for(int index=4; index<extensionPointArrSize; index++){
		initExtendPointArr[pointIndex].setX(extensionPointArr[index*vertexRecordSize]);
		initExtendPointArr[pointIndex].setY(extensionPointArr[index*vertexRecordSize+1]);
		initExtendPointArr[pointIndex].setId(extensionPointIndex);
	...
	//triagulate init points
	for(int index=0; index<initPointArrSize; index++){
		initExtendPointArr[pointIndex].setX(initPointArr[index*vertexRecordSize]);
		initExtendPointArr[pointIndex].setY(initPointArr[index*vertexRecordSize+1]);

		int currPartId = partIndex(initExtendPointArr[pointIndex]);
		startPointIndex = pointIdArr[currPartId];
		pointIdArr[currPartId]++;//this Id for the next point

		initExtendPointArr[pointIndex].setId(startPointIndex);
		pointIndex++;
	}
	...
	qsort(initExtendPointArr, initExtendPointArrSize, sizeof(point), coorX_comparison);


	//init triangulate
	for(unsigned int index=0; index<initExtendPointArrSize; index++){
		basicTriangulate(initExtendPointArr[index]);
//std::cout<<"("<<initExtendPointArr[index].getX()<<", "<<initExtendPointArr[index].getY()<<")  ";
	}

	delete [] initExtendPointArr;
}
