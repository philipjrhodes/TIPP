...
private:
	...
	void writeCoorsToFile(double *coorArr, unsigned int triangleNum, std::string fileStr);
	void writePointIdsToFile(unsigned long long *pointIdArr, unsigned int triangleNum, std::string fileStr);
public:
...
//	unsigned long int *pointIdArr;
	...
	domain(double lowX, double lowY, double highX, double highY, std::string dataNameStr, unsigned int numberOfNodes);
	...
	unsigned int prepareDataForDelaunayMPI(unsigned int coreNum, double &masterTime, double &updateTime);
