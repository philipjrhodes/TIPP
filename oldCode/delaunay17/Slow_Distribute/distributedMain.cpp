...
int main(int argc, char **argv){
	...
	if(argc<12){// no arguments
		...
		nodeNum = atoi(argv[2]);
		xCoarsePartNum = atoi(argv[3]);
		yCoarsePartNum = atoi(argv[4]);
		xFinePartNum = atoi(argv[5]);
		yFinePartNum = atoi(argv[6]);
		...
		std::string dataName = argv[7];
		if(dataName=="") {std::cout<<"The source path does not exist\n"; exit(1);}

		std::string vertexInfoFile = argv[8];
		if(vertexInfoFile=="") {std::cout<<"The vertexInfoFile does not exist\n"; exit(1);}

		std::string vertexDataFile = argv[9];
		...
		distribute *dtb = new distribute(domainSize, nodeNum, xCoarsePartNum, yCoarsePartNum, xFinePartNum, yFinePartNum, dataName, vertexInfoFile, vertexDataFile, chunkSize, initCoarseSize, initFineSize);
