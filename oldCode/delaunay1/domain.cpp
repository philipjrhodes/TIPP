...
//==============================================================================
//check three points forming a triangle or not
//==============================================================================
bool domain::colinear(point p1, point p2, point p3){
...
//	return fabs((y1 - y2) * (x1 - x3) - (y1 - y3) * (x1 - x2))<epsilon;
	return (fabs((x1-x2)*(y2-y3)-(y1-y2)*(x2-x3))<epsilon);
...
void domain::basicTriangulate(point p){
	...
	while(scanEdgeNode!=NULL){
		if(!colinear(scanEdgeNode->ed->p1, scanEdgeNode->ed->p2, p)){
		...
		else std::cout<<"%%%%%%% "<<scanEdgeNode->ed->p1<< scanEdgeNode->ed->p2<< p<<std::endl;
...
std::cout<<"Number of triangles for the new stage: "<<triangleArrSize<<std::endl;

	trianglePartitionList = new std::list<unsigned int>[triangleArrSize];
	if(!trianglePartitionList){std::cout<<"Too many triangles, the memory is overflow!\n";exit(1);}
...
if((partitionEletIdx<0)||(partitionEletIdx>xPartNum*yPartNum-1)){
 std::cout<<"partitionEletIdx: "<<partitionEletIdx<<" i= "<<i<<" j= "<<j<<" xPartNum= "<<xPartNum<<" beginx= "<<beginx<<" beginy= "<<beginy<<" endx= "<<endx<<" endy= "<<endy<<std::endl;
std::cout<<"index= "<<index<<" triangleArrSize= "<<triangleArrSize<<t.p1<<t.p2<<t.p3<<" centerX: "<<centerX<<" centerY: "<<centerY<<", radius: "<<radius<<std::endl;
}
...
// each point: (x, y, Id)
void domain::storeActiveParition(){
	int activePartNum = activePartSet.size();
	//array of active partition Ids 
	int *activePartIdArr = new int[activePartNum];
	int *activePartSizeArr = new int[activePartNum];
	int *activePartSizeOffsetArr = new int[activePartNum];
...
std::cout<<"number of triangles sends to MPI nodes: "<<totalTriangleSize<<"\n";
	...
	//second line stores active partitions
...
std::cout<<"triangleNum: "<<triangleNum<<"\n";
	...
	if(!triangleNewArr){std::cout<<"Running out of memory to allocate triangleNewArr!\n"; exit(1);}
...
//point p1 = triangleNewArr[index].p1;
//point p2 = triangleNewArr[index].p2;
//point p3 = triangleNewArr[index].p3;

//if((p1.getX()<0)||(p1.getX()>1)||(p1.getY()<0)||(p1.getY()>1)||(p2.getX()<0)||(p2.getX()>1)||(p2.getY()<0)||(p2.getY()>1)||(p3.getX()<0)||(p3.getX()>1)||(p3.getY()<0)||(p3.getY()>1)) std::cout<<p1.getX()<<" "<<p1.getY()<<"  "<<p2.getX()<<" "<<p2.getY()<<"   "<<p3.getX()<<" "<<p3.getY()<<std::endl; 
			index++;
		}
...
if((p1.getX()<0)||(p1.getX()>1)||(p1.getY()<0)||(p1.getY()>1)||(p2.getX()<0)||(p2.getX()>1)||(p2.getY()<0)||(p2.getY()>1)||(p3.getX()<0)||(p3.getX()>1)||(p3.getY()<0)||(p3.getY()>1)) std::cout<<p1.getX()<<" "<<p1.getY()<<"  "<<p2.getX()<<" "<<p2.getY()<<"   "<<p3.getX()<<" "<<p3.getY()<<std::endl;
		...
		triangleNewArr[index].p1 = p1;
		triangleNewArr[index].p2 = p2;
		triangleNewArr[index].p3 = p3;

		triangleNewArr[index].computeCenterRadius();

//if(index==48088){
// std::cout<<triangleNewArr[index].p1<<triangleNewArr[index].p2<<triangleNewArr[index].p3<<triangleNewArr[index].centerX<<" "<<triangleNewArr[index].centerY<<" "<<triangleNewArr[index].radius<<std::endl;
//if(colinear(p1, p2, p3)) std::cout<<"Three point p1, p2, p3 is a line\n";
//else std::cout<<"Three points p1, p2, p3 forms a triangle\n";
//}
...
void domain::collectStoreTriangleIds(){
	...
	fseek(f, 0, SEEK_SET); // seek back to beginning of file
std::cout<<"Number of triangles to store:  "<<triangleNum<<std::endl;
	...
	delete [] pointIdArr;
std::cout<<"Store to triangleIds.tri succesfully\n";
}
...
void domain::distributeDelaunay(){
	...
	std::string mpiCommand = "LD_PRELOAD=/usr/libi64/valgrind/vgpreload_core-x86-linux.so  ; mpiexec -n " + std::to_string(activePartNum) + " -f machinefile /usr/bin/valgrind ./delaunayMPI " + path + "delaunayResults/"; 
...
void domain::storeAllTriangles(){
	...
	//triangleArrSize is the number of triangles left
	unsigned long int *tempPointIdArr = new unsigned long int[triangleArrSize*3];

	//copy ids of triangles left from triangleArr to tempPointIdArr
	#pragma omp parallel for
	for(unsigned int index=0; index<triangleArrSize; index++){
