...
//
	double absArea = fabs((x1-x2)*(y2-y3)-(y1-y2)*(x2-x3));
	std::cout.precision(18);
	if(absArea<epsilon) std::cout<<"Abs of area= "<<absArea<<std::endl;
	return absArea<epsilon;
...
//std::cout<<"Edge list after removing bad edges: \n";
		...
		while(scanEdgeNode!=NULL){
			if(colinear(scanEdgeNode->ed->p1, scanEdgeNode->ed->p2, p)==false){
