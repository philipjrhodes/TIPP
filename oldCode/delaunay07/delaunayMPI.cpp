...
float *tempCoorArr=NULL;//all coordinates of triangles by sender
unsigned int *tempPointIdArr=NULL;//all point id of triangles by sender

//store all startId points for each partition
unsigned int *startIdPartArr=NULL;
unsigned int startId;

//all point coordinates of partitions
float *pointCoorArr=NULL;
...
void generateTriangles(int my_rank, int triangleNum, float *coorArr, unsigned int *pointIdArr);
...
//****************************************************************************
	...
	float *coorArr=NULL;//all coordinates of triangles for receivers
	unsigned int *pointIdArr=NULL;//all point id of triangles for receivers
...
//std::cout<<"my Id: "<<my_rank<<"\n";
	...
	MPI_Scatter(startIdPartArr, 1, MPI_UNSIGNED, &startId, 1, MPI_UNSIGNED, MASTER_RANK, MPI_COMM_WORLD);
...
//	    printf("Hello from master process %d on %s of %d\n", my_rank, processor_name, pool_size);
		...
		for(int process_id = 1; process_id < pool_size; process_id++) {//pool_size is number of processes
			ierr = MPI_Send(&tempCoorArr[triangleSizeOffsetArr[process_id]*6], triangleSizeArr[process_id]*6, MPI_FLOAT, process_id, send_data_tag, MPI_COMM_WORLD);
			ierr = MPI_Send(&tempPointIdArr[triangleSizeOffsetArr[process_id]*3], triangleSizeArr[process_id]*3, MPI_UNSIGNED, process_id, send_data_tag, MPI_COMM_WORLD);
		}	
...
//	    printf("Hello from other process %d on %s of %d\n", my_rank, processor_name, pool_size);
		...
		coorArr = new float[triangleSize*6];
		pointIdArr = new unsigned int[triangleSize*3];

//		for(int i=0; i<triangleSize; i++)
//			std::cout<<pointIdArr[3*i]<<"--"<<pointIdArr[3*i+1]<<"--"<<pointIdArr[3*i+2]<<"\n";

		//receive data from master (process 0)
		ierr = MPI_Recv(coorArr, triangleSize*6, MPI_FLOAT, root_process, send_data_tag, MPI_COMM_WORLD, &status);
		ierr = MPI_Recv(pointIdArr, triangleSize*3, MPI_UNSIGNED, root_process, send_data_tag, MPI_COMM_WORLD, &status);
...
//std::cout<<"********triangleNum = "<<triangleNum<<", myrank = "<<my_rank<<std::endl;
	...
	if(triangleNum!=0){//need to store
		...
		unsigned int *triangleIdArr = new unsigned int[triangleNum*3];
...
//		MPI_File_open(MPI_COMM_WORLD, currPath.c_str(), MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &fh);
		MPI_File_open(MPI_COMM_WORLD, currPath.c_str(), MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &fh);
		MPI_File_set_view(fh, storeTriangleNumOffset * 3 * sizeof(unsigned int), MPI_UNSIGNED, MPI_UNSIGNED, "native", MPI_INFO_NULL);
		MPI_File_write(fh, triangleIdArr, triangleNum*3, MPI_UNSIGNED, MPI_STATUS_IGNORE);
		MPI_File_close(&fh);
...
//			std::cout<<triangleNumOffsetArr[i]<<"\n"; 
	...
	if(triangleNum!=0){
		...
		float *pointCoorArr = new float[triangleNum*6];
...
//if(my_rank==3) std::cout<<"triangleNum: "<<triangleNum<<" "<<"triangleNumOffset: "<<triangleNumOffset<<"\n";
		...
		//store array storeTriangleIds of each processes to a single files
		MPI_File_open(MPI_COMM_WORLD, currPath.c_str(), MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &fh);
		MPI_File_set_view(fh, triangleNumOffset * 3 * sizeof(unsigned int), MPI_UNSIGNED, MPI_UNSIGNED, "native", MPI_INFO_NULL);
		MPI_File_write(fh, triangleIdArr, triangleNum*3, MPI_UNSIGNED, MPI_STATUS_IGNORE);
		MPI_File_close(&fh);
		...
		MPI_File_set_view(fh, triangleNumOffset * 6 * sizeof(float), MPI_FLOAT, MPI_FLOAT, "native", MPI_INFO_NULL);
		MPI_File_write(fh, pointCoorArr, triangleNum*6, MPI_FLOAT, MPI_STATUS_IGNORE);
		MPI_File_close(&fh);
...
void readPointCoor(int my_rank){
	...
	if(pointNumPartSize>0){
		...
		pointCoorArr = new float[pointNumPartSize*2];
		//2 mean coordinates x, y
		//PI_File_seek(fh, pointNumPartOffsetSize*2*sizeof(float), MPI_SEEK_SET);
		MPI_File_set_view(fh, pointNumPartOffsetSize*2*sizeof(float), MPI_FLOAT, MPI_FLOAT, "native", MPI_INFO_NULL);
		MPI_File_read(fh, pointCoorArr, pointNumPartSize*2, MPI_FLOAT, &status);
		MPI_File_close(&fh);
...
std::cout<<"\n";
	...
	startIdPartArr = new unsigned int[partNum];
	for(int i=0; i<partNum; i++){
		triangleInfoFile >> strItem;
...
//std::cout<<startIdPartArr[i]<<" ";
	...
	tempCoorArr = new float[totalTriangleSize*6];
	fread(tempCoorArr, sizeof(float), totalTriangleSize*6, f1);
	fclose(f1);
	...
	tempPointIdArr = new unsigned int[totalTriangleSize*3];
	fread(tempPointIdArr, sizeof(float), totalTriangleSize*3, f2);
	fclose(f2);
...
std::cout<<"start triangulation from partId = "<<partId<<std::endl;
	...
	unsigned int partPointId = startId;
