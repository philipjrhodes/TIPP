...
distribute::distribute(int xPartNumber, int yPartNumber, std::string sourcePath, std::string verInfoFile, std::string verFile, std::string resultPath, unsigned int chunksize, int initsize){
	...
	pointGridArr = NULL;
	pointGridArr = NULL;
	partList = new std::list<float>[xPartNum*yPartNum];
...
void distribute::pointsDistribute(){
	...
	pointCoorArr = new float[vertexRecordSize*readingSize];
	if(!pointCoorArr){
...
//	while (readingPos < vertexRecordCount-4){ //not read all points in file mydatabin.ver
	while (readingPos < vertexRecordCount){ //not read all points in file mydatabin.ver
		...
		fread(pointCoorArr, sizeof(float), vertexRecordSize*readingSize, fcoorData);
	...
	float *data = new float[initPointArrSize*vertexRecordSize];
	int index = 0;
...
//std::cout<<data[vertexRecordSize*i]<<" "<<data[vertexRecordSize*i+1]<<std::endl;
	...
	fwrite(data, initPointArrSize*vertexRecordSize, sizeof(float), f);
	fclose(f);
	delete [] data;
...
//==============================================================================
//read billion points from file, partition them into partitions, 
//for ex: a quare domain can be partitioned int to 1 rows and 8 column  -->  8 partitions
//store each partition of points to file
void distribute::pointsDistribute1(){
	FILE *fcoorData=fopen(vertexfilename.c_str(), "rb");
	if(!fcoorData) {std::cout<<"not exist "<<vertexfilename<<std::endl; exit(1);}
...
//std::cout<<vertexRecordCount<<" "<<readingSize<<std::endl;

	//read points from file
	while (readingPos < vertexRecordCount-1){ //not read all points in file mydatabin.ver
		if((readingPos + chunkSize)>vertexRecordCount-1)
			readingSize = vertexRecordCount-readingPos;
//std::cout<<"reading position: "<<readingPos<<std::endl;
		//reading points with readingSize
		fread(pointCoorArr, sizeof(float), vertexRecordSize*readingSize, fcoorData);
		...
		for(unsigned int index=0; index<readingSize; index++){
			point currPoint(pointCoorArr[index*vertexRecordSize], pointCoorArr[index*vertexRecordSize+1]);
			int currPartIndex = partIndex(currPoint);
			//std::cout<<currPartIndex<<std::endl;
			//update pointPartInfo as new point coming
			pointPartInfoArr[currPartIndex]++;
//std::cout<<currPoint.getX()<<" "<<currPoint.getY()<<" "<<currPartIndex<<std::endl;
			//push current point to current partition
			partList[currPartIndex].push_back(currPoint.getX());
			partList[currPartIndex].push_back(currPoint.getY());
		}
//std::cout<<"xPartNum: "<<xPartNum<<std::endl;

		//Write all point coordinates in the same partition into its file
		for(unsigned int i=0; i<xPartNum; i++)
			if(!partList[i].empty()){
				writeListToFile(partList[i], i);
				std::list<float>::iterator it;
//std::cout<<"\npartID "<<i<<": ";
...
//				int j=0;
//				for(it=partList[i].begin(); it!=partList[i].end(); it++){
...
//==============================================================================
void distribute::writeListToFile(std::list<float> pointList, int partitionId){
//std::cout<<"---"<<partitionId<<std::endl;
	unsigned int size = pointList.size();
	float *coorData = new float[size];
...
void distribute::writeToBinaryFile(std::string fileStr, int partitionId, float *data, unsigned int size){
...
void distribute::printPointPartitions(int partitionId){
	...
	if(!f){
		...
		float *data = new float[pointNum*vertexRecordSize];
		fread(data, pointNum*vertexRecordSize, sizeof(float), f);
...
void distribute::printAllPartitions(){
	...
	float *data = new float[allPointNum*vertexRecordSize];
	fread(data, allPointNum*vertexRecordSize, sizeof(float), f);
...
{
  float x1 = ((point*)p1)->getX();
  float x2 = ((point*)p2)->getX();
  return x1 > x2;
}
//==============================================================================
//use qsort of C++
...
void distribute::sort(){
	...
	for(int partId = 0; partId<xPartNum*yPartNum; partId++){
		...
		//	std::cout<<fileStr<<" does not exist\n"; return;}
		//else{
			fread(data, pointNum*vertexRecordSize, sizeof(float), f1);
std::cout<<"sort the content in file ("<<pointNum<<") "<<fileStr<<std::endl;
		...
		fwrite(data, pointNum*vertexRecordSize, sizeof(float), f);
...
void distribute::generateGridPoints(){
	...
	pointGridArr = new float[pointGridArrSize*vertexRecordSize];
...
std::cout<<"size = "<<size<<std::endl;
	...
	float *data = new float[size*vertexRecordSize];
	int index = 0;
	for(int i=0; i<pointGridArrSize; i++){
...
//std::cout<<data[i*vertexRecordSize]<<" "<<data[i*vertexRecordSize+1]<<std::endl;
	//write data to file
	fwrite(data, size*vertexRecordSize, sizeof(float), f);
	fclose(f);
	delete [] data;
...
void distribute::mergeAllPartitionsAndInitPoints(){
	...
	float *initPointArr = new float[initPointArrSize*vertexRecordSize];
	fread(initPointArr, initPointArrSize*vertexRecordSize, sizeof(float), f1);
	fclose(f1);
...
*/
	...
	for(unsigned int partId=0; partId<xPartNum*yPartNum; partId++){
		...
		fwrite(&initPointArr[partId*initSize*vertexRecordSize], initSize*vertexRecordSize, sizeof(float), f);
		...
		fread(data, pointNum*vertexRecordSize, sizeof(float), f2);
	...
	float interval = 1.0/xPartNum;
	float *squarePoints = new float[xPartNum*4*vertexRecordSize];
...
//	float squarePoints[] = {0.0,0.0, 0.0,1.0, 1.0,1.0, 1.0,0.0};
//	fwrite(squarePoints, 4*vertexRecordSize, sizeof(float), f);
	fwrite(squarePoints, sizeof(float), xPartNum*4*vertexRecordSize, f);
//	pointPartInfoFile<<4<<"\n";
	pointPartInfoFile<<xPartNum*yPartNum<<"\n";
