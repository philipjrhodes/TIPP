...
public:
	...
	unsigned int initSize;//the number of points that each partition contributes
...
//	void pointsDistribute1();
	...
	void writeListToFile(std::list<point> pointList, unsigned int partitionId);
	void writeToBinaryFile(std::string fileStr, unsigned int partitionId, point *data, unsigned int size);

	//print points in partitions
	void printPointPartitions(unsigned int partitionId);
	void printPointPartitions1(unsigned int partitionId);
