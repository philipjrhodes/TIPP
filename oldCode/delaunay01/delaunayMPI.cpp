...
//std::cout << "Starting MPI Triangle Writing." << currPath.c_str()  << ", from myrank = "<<my_rank<<std::endl;
MPI_File fh;
MPI_Barrier(MPI_COMM_WORLD);
...
MPI_Barrier(MPI_COMM_WORLD);
		...
		MPI_File_close(&fh);
MPI_Barrier(MPI_COMM_WORLD);	
//std::cout << "Finished MPI Triangle Writing." << currPath.c_str()  << std::endl;
...
MPI_File fh;
	if(pointNumPartSize>0){
		...
		MPI_File_read_all(fh, pointCoorArr, pointNumPartSize*2, MPI_DOUBLE, &status);
		MPI_File_close(&fh);
