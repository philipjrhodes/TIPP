...
std::cout<<"undelivered triangles: "<<count<<"\n";
	...
	//read info from returnStoreTriangleInfo.txt (generated by MPI master)
	std::string fileInfoStr = path + "delaunayResults/returnStoreTriangleInfo.txt";
	std::ifstream returnInfoFile(fileInfoStr.c_str());
	if(!returnInfoFile){
		std::cout<<"non exist "<<fileInfoStr.c_str()<<std::endl;
		exit(1);
	}
	std::string strItem;
	//first line --> read number of active partition
	returnInfoFile >> strItem;
	int activePartNum = atoi(strItem.c_str());

/*	int *activeStorePartArr = new int[activePartNum];
	//second line --> read numbers of stored triangles back from MPI
	for(int i=0; i<activePartNum; i++ ){
		returnInfoFile >> strItem;
		activeStorePartArr[i] = atoi(strItem.c_str());
	}

	int *activeAddPartArr = new int[activePartNum];
	//third line --> read numbers of adding triangles back from MPI
	for(int i=0; i<activePartNum; i++ ){
		returnInfoFile >> strItem;
		activeAddPartArr[i] = atoi(strItem.c_str());
	}
*/
	returnInfoFile.close();

	std::string appendCommand;
	std::string deleteCommand;

	//delete file returnStoreTriangleInfo.txt
	deleteCommand = "rm -f " + path + "delaunayResults/returnStoreTriangleInfo.txt";
...
//	delete [] activeStorePartArr;
//	delete [] activeAddPartArr;
...
std::cout<<"triangleNum: "<<triangleNum<<"\n";
	...
	deleteCommand = "rm -f " + path + "delaunayResults/returnStoreTriangleIds*.tri";
	system(deleteCommand.c_str());
	deleteCommand = "rm -f " + path + "delaunayResults/returnTriangleIds*.tri";
	system(deleteCommand.c_str());
	deleteCommand = "rm -f " + path + "delaunayResults/returnTriangleCoors*.tri";
	system(deleteCommand.c_str());
