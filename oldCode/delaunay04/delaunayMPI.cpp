...
void processStoreTriangles(int my_rank){
	...
	std::string currPath;
	if(my_rank==0){//only master node
		//store info number of stored triangles in each slave node
		std::ofstream infoStoredFile;
		currPath = path + "returnStoreTriangleInfo.txt";
		infoStoredFile.open (currPath.c_str(), std::ofstream::out | std::ofstream::app);
		infoStoredFile << partNum <<"\n";//number of active partitions
		for(int i=0; i<partNum; i++)
			infoStoredFile << storeTriangleNumArr[i] <<" ";
		infoStoredFile <<"\n";
		infoStoredFile.close();
	}
...
//std::cout<<"********triangleNum = "<<triangleNum<<", myrank = "<<my_rank<<std::endl;
	...
	//if triangNum=0 then the MPI running (delaunayMPI) will be stall
...
*/
		...
		currPath = path + "returnStoreTriangleIds" + toString(my_rank) + ".tri";
...
void processTriangleList(int my_rank){
	...
	if(my_rank==0){//only master node
		//store info number of stored triangles in each slave node
		std::ofstream infoStoredFile;
		std::string currPath = path + "returnStoreTriangleInfo.txt";
		infoStoredFile.open (currPath.c_str(), std::ofstream::out | std::ofstream::app);
		for(int i=0; i<partNum; i++)
			infoStoredFile << triangleNumArr[i] <<" ";
		infoStoredFile <<"\n";
		infoStoredFile.close();
	}
	...
	if(triangleNum!=0){
		...
		//save to single file to separate file for each partition
		std::string currPath1 = path + "returnTriangleIds" + toString(my_rank) + ".tri";
		FILE *f = fopen(currPath1.c_str(), "w");
		if(!f){
			std::cout<<"can not write "<<currPath1<<" to disk\n";
			fclose(f);
		}
		fwrite(triangleIdArr, triangleNum*3, sizeof(unsigned long int), f);
		fclose(f);
		...
		//save to single file to separate file for each partition
		std::string currPath2 = path + "returnTriangleCoors" + toString(my_rank) + ".tri";
		f = fopen(currPath2.c_str(), "w");
		if(!f){
			std::cout<<"can not write "<<currPath2<<" to disk\n";
			fclose(f);
		}
		fwrite(pointCoorArr, triangleNum*6, sizeof(double), f);
		fclose(f);
	...
	if(my_rank==0) delete [] triangleNumArr;
}
