...
//============================================================================
double delaunayMPI::processMPI(int my_rank, int pool_size){
	...
	processStoreTime = processStoreTriangles(my_rank, pool_size);

	//store triangleList & temporaryTriangleList from triangulate function
	processTriangleTime = processTriangleList(my_rank, pool_size);

	if(my_rank==MASTER_RANK){
		masterTime += processStoreTime + processTriangleTime;
		delete [] partArr;
...
//============================================================================
void delaunayMPI::storeTriangleIds(unsigned long long *triangleIdArr, unsigned int triangleIdArrSize, std::string fileStr){
...
double delaunayMPI::processStoreTriangles(int my_rank, int pool_size){
...
double delaunayMPI::processTriangleList(int my_rank, int pool_size){
	int ierr;
	double currentTime, masterTime = 0;
	...
	if(my_rank!=MASTER_RANK){//send triangleIdArr to master
		ierr = MPI_Send(triangleIdArr, triangleNum*3, MPI_UNSIGNED_LONG_LONG, MASTER_RANK, send_data_tag, MPI_COMM_WORLD);
		ierr = MPI_Send(pointCoorArr, triangleNum*6, MPI_DOUBLE, MASTER_RANK, send_data_tag, MPI_COMM_WORLD);
	}
	else{//master rank receives all triangleIdArr from other ranks
		currentTime = GetWallClockTime();
		//total size
		...
		//store storeTriangleIdArr to triangleIds.tri (append)
		std::string currPath1 = path + "delaunayResults/returnTriangleIds.tri";
		storeTriangleIds(storeTriangleIdArr, storeTriangleArrSize, currPath1);
		std::string currPath2 = path + "delaunayResults/returnTriangleCoors.tri";
