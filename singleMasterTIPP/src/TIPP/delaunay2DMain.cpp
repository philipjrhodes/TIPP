...
//=============================================================================
	...
	if(argc<=4){// no arguments
		...
		std::cout<<"The fourth argument is the shared folder option (0 or 1)\n";
		return 0;
	}
	...
	domain *d;
	delaunayMPI *pMPI;
	pMPI = new delaunayMPI(domainSize, shareFolderOption, inputPath, outputPath);
	...
	while(!delaunayStop){
		...
		while(!activePartStop){
			if(my_rank==MASTER_RANK){
				...
				d->prepareDataForDelaunayMPI(processNum);
			...
			//process MPI
			pMPI->processMPI(my_rank, pool_size, MPITriangulationTime, MPIReadTime, MPIStoreTime);
