...
void collectStoreTriangleIdArr(unsigned int world_rank, unsigned int world_size, unsigned int row_rank, unsigned int row_size, unsigned int groupNum, unsigned int activeCoarsePartNum, unsigned long long *triangleIdArr, unsigned int triangleNum, std::string path, double &masterTime){
	double currentTime;
	masterTime = 0;
	int ierr;
	MPI_Status status;
...
//=============================================================================
void delaunayPartition(unsigned int world_rank, unsigned int world_size, unsigned int groupNum, unsigned int color, unsigned int row_rank, unsigned int row_size, MPI_Comm row_comm, std::string path, double &masterTime){
...
//				std::cout<<"Number of active partitions leftover: "<<currActivePartNum<<"\n";
	...
	if(row_rank==SUB_MASTER_RANK){
		c->storeAllTriangles(triangleIdArrNode.triangleIdArr, triangleIdArrNode.triangleNum);
		if(triangleIdArrNode.triangleIdArr!=NULL){
			triangleIdArrList.push_back(triangleIdArrNode);
			triangleIdArrNode.triangleIdArr = NULL;
			triangleIdArrNode.triangleNum = 0;	
...
//std::cout<<triangleIdArr[i*3]<<" "<<triangleIdArr[i*3+1]<<" "<<triangleIdArr[i*3+2]<<"  ";
	//collect store triangles (triangleIdArr) from all row_rank = SUB_MASTER_RANK to master_rank = 0
	collectStoreTriangleIdArr(world_rank, world_size, row_rank, row_size, groupNum, activeCoarsePartNum, triangleIdArr, triangleNum, path, amountTime);
	if(world_rank==MASTER_RANK) masterTime += amountTime;
}
...
//std::cout<<"world_rank: " + toString(world_rank) + ", row_rank: " + toString(row_rank) + ", color: " + toString(color) + ", rowSize: " + toString(row_size)<<"\n";
			...
			delaunayPartition(world_rank, world_size, groupNum, color, row_rank, row_size, row_comm, path, amountTime);
			if(world_rank==MASTER_RANK) masterTime += amountTime;
