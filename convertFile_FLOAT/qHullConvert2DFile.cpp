...
void convertFile::writeToBinaryFile(string fileNameStr, double *floatArr, unsigned int floatArrSize){
	FILE *f = fopen(fileNameStr.c_str(), "w");
	fwrite(floatArr, sizeof(double), floatArrSize, f);
	fclose(f);
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
void convertFile::convertVertexFile(string vertexTextFileName, int attributeRecordSize, int *recordSize, unsigned int *recordCount){
	...
	*recordCount = atoi(strItem.c_str());
	...
	unsigned int fileSize = (*recordSize) * (*recordCount);
...
//	else thresholdSize = 4;
	...
	float *data = new float[thresholdSize];
...
//	cf.printfloatFile(path + "/mydatabin.ver", recordSize, recordCount);
