...
//std::cout<<xPartSize<<" "<<yPartSize<<" "<<domainSize<<"\n";
	...
	pointPartInfoArr = new unsigned long int[xPartNum*yPartNum];
	...
	initPointInfoArr = new unsigned int[xPartNum*yPartNum];
...
unsigned int distribute::partIndex(point p){
	double x = p.getX();
	double y = p.getY();
...
void distribute::pointsDistribute(){
	...
	while (readingPos < vertexRecordCount){ //not read all points in file mydatabin.ver
		...
		for(unsigned long long index=0; index<readingSize; index++){
			...
			globalPointIndex++;
			unsigned int currPartIndex = partIndex(currPoint);
...
//==============================================================================
void distribute::writeListToFile(std::list<point> pointList, unsigned int partitionId){
...
void distribute::writeToBinaryFile(std::string fileStr, unsigned int partitionId, point *data, unsigned int size){
...
void distribute::printPointPartitions1(unsigned int partitionId){

	//form a file name from partitionId (partitionId = 5 --> pointPart05.ver)
	std::string fileStr = generateFileName(partitionId, destPath + "pointPart", xPartNum*yPartNum);
//	std::string fileStr = destPath + "pointCoorPart.ver";
...
void distribute::appendTwoFiles(point *pointArr, unsigned int pointNum){
	...
	unsigned long long *pointIdArr = new unsigned long long[pointNum];
	...
	fwrite(pointIdArr, pointNum, sizeof(unsigned long long), f2);
	fclose(f2);
