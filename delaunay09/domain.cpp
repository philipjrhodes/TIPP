...
std::cout<<"Number of partition points (not include initial points): "<<pointNumMax<<"\n";
	...
	//fourth line: the total number of initial points
	vertexPartInfoFile >> strItem;
...
std::cout<<"Total number of points in domain: "<<pointNum<<"\n";
	...
	//fifth line: number of grid points (not includes 4 corner points)
	vertexPartInfoFile >> strItem;
...
void domain::generateActivePartitions(){
	...
	bool *activePartArr;
	try {
		activePartArr = new bool[xPartNum*yPartNum];
	} catch (std::bad_alloc&) {
	  // Handle error
		std::cout<<"Memory overflow!!!!!!!!!!!\n";
		exit(1);
...
double domain::processDelaunayMPI(int coreNum){
	int *activePartIdArr;
	...
	int activePartNumber = activePartNum;
...
//std::cout<<indexPartId<<", localIndexPartId: "<<localIndexPartId<<", partId: "<<partId<<", totalTriangleSize: "<<totalTriangleSize<<"activePartNum: "<<activePartNum<<", AAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n";
		...
		//if coreNum < totalActivePartNum, then reset activePartIdArr, activePartSizeArr
		//store ro files, and call MPI
		if(((indexPartId + 1) % activePartNumber == 0)||(indexPartId == totalActivePartNum-1)){
...
//std::cout<<"activePartNum: "<<activePartNum<<", totalTriangleSize: "<<totalTriangleSize<<"\n";
...
//std::cout<<"maximum number of active partitions: "<<totalActivePartNum<<"\n";

			time = GetWallClockTime() - time;
			totalTime += time;
			
			//run MPI command to process Delaunay Triangulation on slave nodes
			runDelaunayMPI(domainSize, activePartNum);

			time = GetWallClockTime();

			//add returnStoreTrianglesIds.tri to triangleIds.tri
			collectStoreTriangleIds();
			...
			if(totalActivePartNum - indexPartId <= activePartNum)
				activePartNum = totalActivePartNum - indexPartId - 1;
...
void domain::storeActiveParitionInfo(int activePartNum, int *activePartIdArr, int *activePartSizeArr, int *activePartSizeOffsetArr){
	...
	int *pointPartInfoOffsetArr;
	try {
		pointPartInfoOffsetArr = new int[xPartNum*yPartNum];
	} catch (std::bad_alloc&) {
	  // Handle error
		std::cout<<"Memory overflow!!!!!!!!!!!\n";
		exit(1);
...
/*
 //draw inactive triangles not including triangles returning back from MPI
	drawMesh *d = new drawMesh;
	d->	drawGridLines(xPartNum, yPartNum);
	d->drawTriangleArr(triangleNewArr, count, 2);//GREEN
	delete d;
*/
...
//=========================================================================================
//Use files return from MPI, draw return and store triangles.
//You have to temporary comment away collectStoreTriangleIds() in processDelaunayMPI(coreNum);
void domain::drawReturnAndStoreTriangles(){
	//read returnAllTriangleCoors.tri and returnAllTriangleIds.tri
	std::string fileStr = path + "delaunayResults/returnAllTriangleCoors.tri";
	...
	fread(tempPointIdArr, triangleNum*3, sizeof(unsigned long long), f);
	fclose(f);

	triangle *returnTriangleArr;
	try {
		returnTriangleArr = new triangle[triangleNum];
	} catch (std::bad_alloc&) {
	  // Handle error
		std::cout<<"Memory overflow!!!!!!!!!!!\n";
		exit(1);
	}
	//based on two new arrays tempCoorArr & tempPointIdArr, populate info for the rest of triangles in triangleArr1
	for(int i=0; i<triangleNum; i++){
		point p1(tempCoorArr[i*6], tempCoorArr[i*6+1], tempPointIdArr[i*3]);
		point p2(tempCoorArr[i*6+2], tempCoorArr[i*6+3], tempPointIdArr[i*3+1]);
		point p3(tempCoorArr[i*6+4], tempCoorArr[i*6+5], tempPointIdArr[i*3+2]);
		...
		//assign new info
		returnTriangleArr[i].delivered = false;
		returnTriangleArr[i].p1.set(p1);
		returnTriangleArr[i].p2.set(p2);
		returnTriangleArr[i].p3.set(p3);
	}
	delete [] tempCoorArr;
	delete [] tempPointIdArr;
...
//---------------------------------------------------------------------
	...
	double *pointCoorArr;
	try {
		pointCoorArr = new double[size*2];
	} catch (std::bad_alloc&) {
	  // Handle error
		std::cout<<"Memory overflow!!!!!!!!!!!\n";
		exit(1);
	...
	fileStr = path + "delaunayResults/fullPointPart.ver";
	...
	//read returnStoreTriangleIds.tri
	fileStr1 = path + "delaunayResults/returnStoreTriangleIds.tri";
	f = fopen(fileStr1.c_str(), "rb");
	if(!f){
		std::cout<<"not exist "<<fileStr1<<std::endl;
		return;
	...
	fseek(f, 0, SEEK_END); // seek to end of file
	triangleNum = ftell(f)/(3*sizeof(unsigned long long)); 
	fseek(f, 0, SEEK_SET); // seek back to beginning of file

	unsigned long long *pointIdArr;
	try {
		pointIdArr = new unsigned long long[triangleNum*3];
	} catch (std::bad_alloc&) {
	  // Handle error
		std::cout<<"Memory overflow!!!!!!!!!!!\n";
		exit(1);
	...
	triangle *storeTriangleArr;
	try {
		storeTriangleArr = new triangle[triangleNum];
	} catch (std::bad_alloc&) {
	  // Handle error
		std::cout<<"Memory overflow!!!!!!!!!!!\n";
		exit(1);
	...
	drawMesh *d = new drawMesh;
	d->	drawGridLines(xPartNum, yPartNum);
	d->drawTriangleArr(storeTriangleArr, triangleNum, 3);//CYAN
	d->drawTriangleArr(returnTriangleArr, triangleNum, 2);//GREEN

	delete [] returnTriangleArr;
	delete [] pointIdArr;
	delete [] pointCoorArr;
...
void domain::collectStoreTriangleIds(){
	...
	fseek(f, 0, SEEK_END); // seek to end of file
	unsigned int triangleNum = ftell(f)/(3*sizeof(unsigned long long)); 
	fseek(f, 0, SEEK_SET); // seek back to beginning of file
	...
	//remove returnStoreTriangleIds.tri
	...
	fwrite(pointIdArr, triangleNum*3, sizeof(unsigned long long), f1);
	fclose(f1);
...
//===================================================================
//draw triangle in the linklist triangleList and temporaryTriangleList
...
*/
	drawMesh *d = new drawMesh;
	d->	drawGridLines(xPartNum, yPartNum);
	if(triangleList!=NULL)	d->drawTriangles(triangleList, 3);
	if(temporaryTriangleList!=NULL)	d->drawTriangles(temporaryTriangleList, 3);
	delete d;
...
boundingBox domain::partBox(int partId){
	double xPartSize = domainSize/xPartNum;
	double yPartSize = domainSize/yPartNum;
...
std::cout<<gridPartX*xPartSize<<" "<<gridPartY*yPartSize<<" "<<(gridPartX+1)*xPartSize<<" "<<(gridPartY+1)*yPartSize<<"\n";
	return boundingBox(lowPoint, highPoint);
}

//===================================================================
void domain::drawBoundaryTriangleArr(int partId){
	//read fullPointPart.ver to pointCoorArr
std::cout<<"************pointNumMax: "<<pointNumMax<<std::endl;
	unsigned int size = (pointNumMax+4);
	double *pointCoorArr = new double[size*2];
	...
	//build storeTriangleArr
	triangle *allTriangleArr = new triangle[triangleNum];
	triangle *boundaryTriangleArr = new triangle[triangleNum];
	...
	for(int i=0; i<triangleNum; i++){
		...
		allTriangleArr[i].p1.setId(pointId1);
		allTriangleArr[i].p2.setId(pointId2);
		allTriangleArr[i].p3.setId(pointId3);

		allTriangleArr[i].p1.setX(pointCoorArr[pointId1*2]);
		allTriangleArr[i].p1.setY(pointCoorArr[pointId1*2+1]);

		allTriangleArr[i].p2.setX(pointCoorArr[pointId2*2]);
		allTriangleArr[i].p2.setY(pointCoorArr[pointId2*2+1]);

		allTriangleArr[i].p3.setX(pointCoorArr[pointId3*2]);
		allTriangleArr[i].p3.setY(pointCoorArr[pointId3*2+1]);
		allTriangleArr[i].computeCenterRadius();
	}

	int index=0;
	boundingBox currBox = partBox(partId);
	for(int i=0; i<triangleNum; i++){
		if(allTriangleArr[i].intersect(currBox)){
			boundaryTriangleArr[index] = allTriangleArr[i];
			index++;
		}
	}
	unsigned int boundaryTriangleSize = index;


	drawMesh *d = new drawMesh;
	d->	drawGridLines(xPartNum, yPartNum);
	d->drawTriangleArr(boundaryTriangleArr, boundaryTriangleSize, 3);//CYAN
//	d->drawTriangleArr(allTriangleArr, triangleNum, 3);//CYAN

	delete d;

	delete [] pointCoorArr;
	delete [] pointIdArr;
	delete [] allTriangleArr;	
	delete [] boundaryTriangleArr;
}

//===================================================================
//Based on all triangles ids in triangleIds.tri, draw all triangles (not include triangles with grid points)
void domain::drawTriangleArr1(){
	//read all points from mydatabin.ver
	//pointNum is the total number of points in domain (not include grid points)
	double *pointCoorArr = new double[pointNum*2];
	std::string fileStr = path + "rawPointData/mydatabin.ver";
	FILE *f = fopen(fileStr.c_str(), "rb");
	if(!f){
		std::cout<<"not exist "<<fileStr<<std::endl;
		return;
	...
	fseek(f1, 0, SEEK_END); // seek to end of file
	//totalTriangleNum is total number of triangles include those triangles with grid points
	unsigned long long totalTriangleNum = ftell(f1)/(3*sizeof(unsigned long long)); // get current file pointer
	fseek(f1, 0, SEEK_SET); // seek back to beginning of file

	unsigned long long *pointIdArr = new unsigned long long[totalTriangleNum*3];
	fread(pointIdArr, totalTriangleNum*3, sizeof(unsigned long long), f1);
	fclose(f1);
...
*/
	...
	//triangleNum is the total number of triangles without grid points
	unsigned long long triangleNum = 0;
	//build storeTriangleArr
	triangle *storeTriangleArr = new triangle[totalTriangleNum];
	for(int i=0; i<totalTriangleNum; i++){
		...
		if((pointId1>=pointNum)||(pointId2>=pointNum)||(pointId3>=pointNum))
			continue;

		storeTriangleArr[index].p1.setId(pointId1);
		storeTriangleArr[index].p2.setId(pointId2);
		storeTriangleArr[index].p3.setId(pointId3);

		storeTriangleArr[index].p1.setX(pointCoorArr[pointId1*2]);
		storeTriangleArr[index].p1.setY(pointCoorArr[pointId1*2+1]);

		storeTriangleArr[index].p2.setX(pointCoorArr[pointId2*2]);
		storeTriangleArr[index].p2.setY(pointCoorArr[pointId2*2+1]);

		storeTriangleArr[index].p3.setX(pointCoorArr[pointId3*2]);
		storeTriangleArr[index].p3.setY(pointCoorArr[pointId3*2+1]);
//std::cout<<triangleArr[index].p1.getX()<<" "<<triangleArr[index].p1.getY()<<" "<<triangleArr[index].p2.getX()<<" "<<triangleArr[index].p2.getY()<<" "<<triangleArr[index].p3.getX()<<" "<<triangleArr[index].p3.getY()<<"\n";
		storeTriangleArr[index].computeCenterRadius();
		index++;
	}
	triangleNum = index - 1;
	std::cout<<"Number of triangles in domain without grid points: "<<triangleNum<<std::endl;
	drawMesh *d = new drawMesh;
	d->	drawGridLines(xPartNum, yPartNum);
	d->drawTriangleArr(storeTriangleArr, triangleNum, 2);//GREEN
//	if(triangleArr!=NULL) d->drawTriangleArr(triangleArr, triangleArrSize, 3);//CYAN
...
//===================================================================
//draw triangles left over in memory after storing some finished triangles
void domain::drawLeftOverTriangleArr(){
	drawMesh *d = new drawMesh;
	d->	drawGridLines(xPartNum, yPartNum);
	if(triangleArr!=NULL) d->drawTriangleArr(triangleArr, triangleArrSize, 3);//CYAN
	delete d;
