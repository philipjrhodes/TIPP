...
distribute::distribute(double domainsize, unsigned int numberOfNodes, unsigned int xCoarsePartNumber, unsigned int yCoarsePartNumber, unsigned int xFinePartNumber, unsigned int yFinePartNumber, std::string dataNameStr, std::string verInfoFile, std::string verFile, unsigned long long chunksize, unsigned int initcoarsesize, unsigned int initfinesize){
	...
	path = "/data0/" + dataName;
	vertexInfofilename = path + "/rawPointData/" + verInfoFile;
	vertexfilename = path + "/rawPointData/" + verFile;
	chunkSize = chunksize;
...
void distribute::processInitPoints(){
	...
	for(unsigned int coarsePartId=0; coarsePartId<xCoarsePartNum*yCoarsePartNum; coarsePartId++){
		...
		for(unsigned int finePartId=0; finePartId<xFinePartNum*yFinePartNum; finePartId++)
			if(initPointArr[coarsePartId][finePartId].size()<(initCoarseSize+initFineSize)){
				...
				initPartFileStr = generateFileName(coarsePartId, path + "/initPointPart", xCoarsePartNum*yCoarsePartNum, ".ver");
	...
	std::string initDomainFileStr = path + "/initDomainPoints.ver";
...
void distribute::createInfoFiles(){
	...
	std::ofstream pointPartInfoFile(path + "/pointDomainInfo.xfdl", std::ofstream::out | std::ofstream::trunc);
	...
	std::string	folderShare;
	for(unsigned int coarsePartId=0; coarsePartId<xCoarsePartNum*yCoarsePartNum; coarsePartId++){
		fileStr = generateFileName(coarsePartId, path + "/pointPartInfo", xCoarsePartNum*yCoarsePartNum, ".xfdl");
		//its previous content is deleted and replaced by the new one.
...
void distribute::writeListToFile(std::list<point> pointList, unsigned int coarsePartId, unsigned int finePartId){
	...
	writeToBinaryFile(path + "/pointPart", coarsePartId, finePartId, pointArr, size);
	delete [] pointArr;
}

//==============================================================================
//append data with size to a binary file, for ex: with partition1 in 16 partitions, the file name would be
// "pointPartxx.ver" in --> "/data0/10vertices/pointPartitions"
...
//	#pragma omp parallel for
	for(unsigned int coarsePartId=0; coarsePartId<xCoarsePartNum*yCoarsePartNum; coarsePartId++)
		for(unsigned int finePartId=0; finePartId<xFinePartNum*yFinePartNum; finePartId++){
			std::string fileStr = generateFileName(coarsePartId, path + "/pointPart", xCoarsePartNum*yCoarsePartNum, "");
			fileStr = generateFileName(finePartId, fileStr + "_", xFinePartNum*yFinePartNum, ".ver");
...
//distribute files (initPointPartXX.ver, pointPartInfoXX.xfdl, pointPartXX_XX.ver) to shareFolders
void distribute::distributeFiles(){
	std::string mvCommand1, mvCommand2, mvCommand3, sourceFileName, folderShare;
	std::cout<<"number of nnodes in cluster: "<<nodeNum<<"\n";
	std::cout<<"start copying data files in folder share /data0 to /dataN ...\n";
	for(unsigned int coarsePartId=0; coarsePartId<xCoarsePartNum*yCoarsePartNum; coarsePartId++){
		if((coarsePartId % nodeNum) !=0){
			folderShare = "/data" + toString(coarsePartId % nodeNum) + "/";
			sourceFileName = generateFileName(coarsePartId, "/pointPart", xCoarsePartNum*yCoarsePartNum, "");
			mvCommand1 = "mv /data0/" + dataName + sourceFileName + "* " + folderShare + dataName + "/";
			std::cout<<mvCommand1<<std::endl;
			system(mvCommand1.c_str());

			sourceFileName = generateFileName(coarsePartId, "/initPointPart", xCoarsePartNum*yCoarsePartNum, ".ver");
			mvCommand2 = "mv /data0/" + dataName + sourceFileName + " " + folderShare + dataName + "/";
			std::cout<<mvCommand2<<std::endl;
			system(mvCommand2.c_str());

			sourceFileName = generateFileName(coarsePartId, "/pointPartInfo", xCoarsePartNum*yCoarsePartNum, ".xfdl");
			mvCommand3 = "mv /data0/" + dataName + sourceFileName + " " + folderShare + dataName + "/";
			std::cout<<mvCommand3<<std::endl;
			system(mvCommand3.c_str());
		}
	}
}
