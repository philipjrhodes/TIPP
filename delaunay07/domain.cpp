...
//===================================================================
domain::domain(float lowX, float lowY, float highX, float highY, std::string pathStr){
	lowPoint.setX(lowX);
	lowPoint.setY(lowY);
...
domain::domain(float lowX, float lowY, float highX, float highY){
	lowPoint.setX(lowX);
	lowPoint.setY(lowY);
	highPoint.setX(highX);
	highPoint.setY(highY);
...
float domain::getDomainSizeX(){
	return (highPoint.getX() - lowPoint.getX());
}
float domain::getDomainSizeY(){
...
std::cout<<"pointSum: "<<pointSum<<", pointNumMax: "<<pointNumMax<<"\n";
	...
	pointIdArr = new unsigned int[xPartNum*yPartNum+1];
...
//std::cout<<"initPointArrSize: "<<initPointArrSize<<"\n";
	initPointArr = new float[initPointArrSize*vertexRecordSize];
	fread(initPointArr, sizeof(float), initPointArrSize*vertexRecordSize, f);
	fclose(f);
}
...
void domain::loadExtensionPoints(){
	...
	fseek(f, pointNumMax*sizeof(float)*vertexRecordSize, SEEK_CUR); // move to a specific location
	extensionPointArr = new float[extensionPointArrSize*vertexRecordSize];

	//read all extension points (grid points on 4 edges of domain, incluing 3 corners)
	fread(extensionPointArr, sizeof(float), extensionPointArrSize*vertexRecordSize, f);
	fclose(f);
...
int domain::partIndex(point p){
	...
	float xPartSize = 1.0/xPartNum;
	float yPartSize = 1.0/yPartNum;
...
void domain::initTriangulate(){
	...
	unsigned int extensionPointIndex = pointNumMax+4;
...
void domain::generateIntersection(){
	float globalLowX = geoBound.getLowPoint().getX();
	float globalLowY = geoBound.getLowPoint().getY();
...
std::cout<<"triangleIdList is empty\n";
	...
	unsigned int *data = new unsigned int[size*3];
	unsigned int index = 0;
	...
	fwrite(data, size*3, sizeof(unsigned int), f);
	fclose(f);
	delete [] data;
...
double domain::processDelaunayMPI(int coreNum){
	...
	for (std::set<int>::iterator it=activePartSet.begin(); it!=activePartSet.end(); ++it){
		...
		if(((indexPartId + 1) % activePartNum == 0)||(indexPartId == totalActivePartNum-1)){
...
void domain::storeActiveParitions(int *activePartIdArr, int activePartNum, unsigned int totalTriangleSize){
	...
	float *tempCoorArr = new float[totalTriangleSize*6];//contain three point coordinates
	unsigned int *tempPointIdArr = new unsigned int[totalTriangleSize*3];//contain three point Ids
	int triangleIndex = 0;
...
std::cout<<"number of triangles sends to MPI nodes: "<<totalTriangleSize<<"\n";
	...
	fwrite(tempCoorArr, totalTriangleSize*6, sizeof(float), f1);
	fclose(f1);
std::cout<<"DONE! storeActiveParitions\n";
	...
	fwrite(tempPointIdArr, totalTriangleSize*3, sizeof(unsigned int), f2);
	fclose(f2);
...
std::cout<<"undelivered triangles: "<<count<<"\n";
	...
	unsigned int triangleNum = ftell(f)/(6*sizeof(float)); // get current file pointer
	fseek(f, 0, SEEK_SET); // seek back to beginning of file
std::cout<<"triangleNum: "<<triangleNum<<"\n";
	float *tempCoorArr = new float[triangleNum*6];
	fread(tempCoorArr, triangleNum*6, sizeof(float), f);
	fclose(f);
	...
	unsigned int *tempPointIdArr = new unsigned int[triangleNum*3];
	fread(tempPointIdArr, triangleNum*3, sizeof(unsigned int), f1);
	fclose(f1);
...
void domain::collectStoreTriangleIds(){
	...
	fseek(f, 0, SEEK_END); // seek to end of file
	unsigned int triangleNum = ftell(f)/(3*sizeof(unsigned int)); 
	fseek(f, 0, SEEK_SET); // seek back to beginning of file

	unsigned int *pointIdArr = new unsigned int[triangleNum*3];
	fread(pointIdArr, triangleNum*3, sizeof(unsigned int), f);
	fclose(f);
	...
	fwrite(pointIdArr, triangleNum*3, sizeof(unsigned int), f1);
	fclose(f1);
...
void domain::runDelaunayMPI(int activePartNum){
	...
	std::string mpiCommand = "mpiexec -n " + std::to_string(activePartNum) + "  ./delaunayMPI " + path + "delaunayResults/";
...
void domain::storeAllTriangles(){
	...
	unsigned int *tempPointIdArr = new unsigned int[triangleArrSize*3];
	...
	fwrite(tempPointIdArr, triangleArrSize*3, sizeof(unsigned int), f);
	fclose(f);
...
void domain::drawTriangleArr(){
	...
	float *pointCoorArr = new float[size*2];
	...
	fread(pointCoorArr, size*2, sizeof(float), f);
	fclose(f);
	...
	unsigned int triangleNum = ftell(f1)/(3*sizeof(unsigned int)); // get current file pointer
	fseek(f1, 0, SEEK_SET); // seek back to beginning of file

	unsigned int *pointIdArr = new unsigned int[triangleNum*3];
	fread(pointIdArr, triangleNum*3, sizeof(unsigned int), f1);
	fclose(f1);
...
gridElement domain::mapHigh(boundingBox bBox, float gridElementSizeX, float gridElementSizeY){
	int gridx, gridy;
...
gridElement domain::mapLow(boundingBox bBox, float gridElementSizeX, float gridElementSizeY){
	int gridx, gridy;
	...
	if(bBox.getHighPoint().getX() > geoBound.getHighPoint().getX())
		...
		float v = (bBox.getHighPoint().getX() - geoBound.getLowPoint().getX())/gridElementSizeX;
	...
	if(bBox.getHighPoint().getY() > geoBound.getHighPoint().getY())
		...
		float v = (bBox.getHighPoint().getY() - geoBound.getLowPoint().getY())/gridElementSizeY;
