...
public:
	...
	float *extensionPointArr;
	unsigned int extensionPointArrSize;
	

	//total number of point in domain (wihtout 4 points in the corners of domain square)
	unsigned int pointNumMax;

	domain(float lowX, float lowY, float highX, float highY, std::string pathStr);
	domain(float lowX, float lowY, float highX, float highY);
//	domain(const point &lPoint, const point &hPoint, std::string pathStr);
	...
	float getDomainSizeX();
	float getDomainSizeY();
	...
	geoBound is is the domain bound*/
	 gridElement mapHigh(boundingBox bBox, float gridElementSizeX, float gridElementSizeY);
	...
	/** Map a point on the partitioning. If p falls on a partition boundary, we choose the
	 ...
	 gridElement mapLow(boundingBox bBox, float gridElementSizeX, float gridElementSizeY);
	...
	double processDelaunayMPI(int coreNum);
	void storeActiveParitions(int *activePartIdArr, int activePartNum, unsigned int totalTriangleSize);
	void storeActiveParitionInfo(int activePartNum, int *activePartIdArr, int *activePartSizeArr, int *activePartSizeOffsetArr);
	...
	void runDelaunayMPI(int activePartNum);
