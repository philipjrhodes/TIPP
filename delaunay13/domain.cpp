...
void domain::prepareDataForDelaunayMPI(int coreNum){
	...
	try {
		activePartIdArr = new int[currActivePartNum];
	} catch (std::bad_alloc&) {
	  // Handle error
		std::cout<<"Memory overflow!!!!!!!!!!!\n";
		exit(1);
	...
	try {
		activePartSizeArr = new int[currActivePartNum];
	} catch (std::bad_alloc&) {
	  // Handle error
		std::cout<<"Memory overflow!!!!!!!!!!!\n";
		exit(1);
	...
	try {
		activePartSizeOffsetArr = new int[currActivePartNum];
	} catch (std::bad_alloc&) {
	  // Handle error
		std::cout<<"Memory overflow!!!!!!!!!!!\n";
		exit(1);
...
//std::cout<<"------------ Number of active partition left over: "<<activePartSet.size()<<" ----------\n";
	...
	for(int i=1; i<currActivePartNum; i++)
...
void domain::storeActivePartitions(int *activePartIdArr, int activePartNum, unsigned int totalTriangleSize){
...
void domain::storeActivePartitionInfo(int activePartNum, int *activePartIdArr, int *activePartSizeArr, int *activePartSizeOffsetArr){
	//stores meta data
...
std::cout<<"triangleNum: "<<triangleNum<<"\n";
	...
	//remove returnTriangleCoors.tri, and returnTriangleIds.tri
	delCommand = "rm -f " + path + "delaunayResults/returnStoreTriangleIds*.tri";
	system(delCommand.c_str());
	delCommand = "rm -f " + path + "delaunayResults/returnTriangleIds*.tri";
	system(delCommand.c_str());
	delCommand = "rm -f " + path + "delaunayResults/returnTriangleCoors*.tri";
	system(delCommand.c_str());
