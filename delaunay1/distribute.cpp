...
// "pointPartxx.ver" in --> "./dataSources/10vertices/delaunayResults"
void distribute::writeToBinaryFile(std::string fileStr, int partitionId, double *data, unsigned int size){
	...
	FILE *f = fopen(fileStr.c_str(), "a");//appending i
...
*/
	...
	//write 4 points of square (0,0), (0,1), (1,1), (1,0) in the end of fullPointPartArr.ver
	double squarePoints[] = {0.0,0.0, 0.0,1.0, 1.0,1.0, 1.0,0.0};
	fwrite(squarePoints, 4*vertexRecordSize, sizeof(double), f);
	pointPartInfoFile<<4<<"\n";

	//generate pointNumOffsetArr offset for partition sizes (includes init points), 1 mean the last part contains 4 points of square domain
	//update pointIdArr
	unsigned int pointNumOffset = 0;
	pointPartInfoFile<<pointNumOffset<<" ";
