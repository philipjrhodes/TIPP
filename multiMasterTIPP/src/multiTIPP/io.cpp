...
void readDataFile(std::string triangleIdFileName, std::string triangleCoorFileName, unsigned long long readingPos, unsigned long int &readingSize, unsigned long long totalTriangleNum, unsigned long long *&triangleIdArr, double *&triangleCoorArr){
	...
	if(readingPos > totalTriangleNum){
		std::cout<<"readingPos must be smaller than totalTriangleNum\n";
		exit(-1);
	}
...
void readTriangleCoors(double *&triangleCoorArr, unsigned long long &triangleNum, std::string fileStr){
	...
	if(!f){
		...
		triangleNum = 0;
		quit("not success to open " + fileStr);
	}
	fseek(f, 0, SEEK_END); // seek to end of file
...
void readPoint_withoutAllocation(point *pointArr, unsigned int pointNum, std::string currPath){
	...
	FILE *f = fopen(currPath.c_str(), "rb");
	if(!f){
		std::cout<<"not exist "<<currPath<<", pointNum: "<<pointNum<<std::endl;
		return;
	}
	fread(pointArr, sizeof(point), pointNum, f);
	fclose(f);
...
unsigned long long appendFile(std::string fileNameStr1, std::string fileNameStr2){
	...
	if(!f){
		std::cout<<"not exist "<<fileNameStr1<<std::endl;
		return 0; 
	}
	fseek(f, 0, SEEK_END); // seek to end of file
	...
	if(!f){
		std::cout<<"not exist "<<fileNameStr2<<std::endl;
		return 0;
	}
	fwrite(triangleIdArr, triangleNum*3, sizeof(unsigned long long), f);
	fclose(f);
	delete [] triangleIdArr;
	//std::cout<<"appending "<<fileNameStr1<<" to "<<fileNameStr2<<"\n";
	return triangleNum;
}
...
unsigned long long combineSubFiles(std::string dstPath, std::string fileNameStr){
	...
	unsigned long long totalAllSubTriangleNum=0;
	...
	for(std::list<std::string>::iterator it=processFiles.begin(); it!=processFiles.end(); it++){
		totalAllSubTriangleNum += appendFile(dstPath + (*it), dstfile);
		//std::cout<<(*it)<<" ";
	}
	return totalAllSubTriangleNum;
}
...
*/
//============================================================================
//convert subFileTriangleList to flat array (combine multiple subArr into one big array)
void combineListArr(std::list<subFileTriangle *> &subFileTriangleList, unsigned long long *&totalTriangleIdArr, unsigned &totalSubTriangleNum){
	totalSubTriangleNum = 0;
	for(std::list<subFileTriangle *>::iterator it=subFileTriangleList.begin(); it!=subFileTriangleList.end(); it++)
		totalSubTriangleNum += (*it)->subTriangleNum;
	if(totalSubTriangleNum<=0) return;

	totalTriangleIdArr = new unsigned long long[totalSubTriangleNum*3];
	unsigned index = 0;
	for(std::list<subFileTriangle *>::iterator it=subFileTriangleList.begin(); it!=subFileTriangleList.end(); it++){
		unsigned long long *subTriangleIdArr = (*it)->subTriangleIdArr;
		unsigned subTriangleNum = (*it)->subTriangleNum;

		for(unsigned i=0; i<subTriangleNum; i++){
			totalTriangleIdArr[index*3] = subTriangleIdArr[i*3];
			totalTriangleIdArr[index*3+1] = subTriangleIdArr[i*3+1];
			totalTriangleIdArr[index*3+2] = subTriangleIdArr[i*3+2];
			index++;
		}
		delete [] subTriangleIdArr;
		delete (*it);
	}
	subFileTriangleList.clear();
}

//============================================================================
//if fileNameStr =="001", we read all subfiles (001_0.tri, 001_2.tri, 001_3.tri) --> triangleIdArr, triangleNum
void readSubFiles(std::string dstPath, std::string fileNameStr, unsigned long long *&totalTriangleIdArr, unsigned &totalSubTriangleNum){
	...
	std::list<subFileTriangle *> subFileTriangleList;
	for(std::list<std::string>::iterator it=processFiles.begin(); it!=processFiles.end(); it++){
		subFileTriangle *sft = new subFileTriangle;
		readTriangleIds(sft->subTriangleIdArr, sft->subTriangleNum, dstPath + (*it));
		subFileTriangleList.push_back(sft);
		//totalSubTriangleNum += sft.subTriangleNum;
		//std::cout<<(*it)<<" ";
	}

	combineListArr(subFileTriangleList, totalTriangleIdArr, totalSubTriangleNum);

/*	totalTriangleIdArr = new unsigned long long[totalSubTriangleNum*3];
	unsigned index = 0;
	for(std::list<subFileTriangle *>::iterator it=subFileTriangleList.begin(); it!=subFileTriangleList.end(); it++){
		unsigned long long *subTriangleArr = (*it)->subTriangleArr;
		unsigned subTriangleNum = (*it)->subTriangleNum;

		for(unsigned i=0; i<subTriangleNum; i++){
			totalTriangleIdArr[index*3] = subTriangleArr[i*3];
			totalTriangleIdArr[index*3+1] = subTriangleArr[i*3+1];
			totalTriangleIdArr[index*3+2] = subTriangleArr[i*3+2];
			index++;
		}
		deallocateMemory(subTriangleArr);
	}
	subFileTriangleList.clear();
*/
}
