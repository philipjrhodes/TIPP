...
int main(int argc, char **argv){
	...
	if(argc<6){// no arguments
		...
		unsigned int pointNumThreshold = atoi(argv[3]);
		unsigned int totalPointNum = atoll(argv[4]);

		std::string destPath = argv[5];
		if(destPath=="") {std::cout<<"The destination path does not exist\n"; exit(1);}

		unsigned int initCoarsePointNum = atoi(argv[6]);
		unsigned int initFinePointNum = atoi(argv[7]);

		distribute *dtb = new distribute(rowCoarsePartNum, colCoarsePartNum, pointNumThreshold, totalPointNum, initCoarsePointNum, initFinePointNum, destPath);
		dtb->readMapData();
		dtb->calculatePointCoarsePartNum();
		dtb->generatePointsAllCoarsePartitions();
		dtb->addGridPointsToInitPoints();
		dtb->createFullPointCoorData();

		dtb->readFullPointPartData();
//for(int j=0; j<25; j++)	dtb->printPointPart(15,j);
		dtb->testData();
//dtb->printPointPart(4,2);
		delete dtb;
		}
	std::cout<<"done!\n";
	return 0;
