...
int main(int argc, char **argv){
	...
	if(argc<6){// no arguments
		...
		unsigned int totalPointNum = atoll(argv[5]);
		...
		distribute *dtb = new distribute(rowCoarsePartNum, colCoarsePartNum, rowFinePartNum, colFinePartNum, totalPointNum, initCoarsePointNum, initFinePointNum, destPath);
		dtb->readMapData();
		dtb->calculatePointPartNum();
		dtb->generateScalePoints();
