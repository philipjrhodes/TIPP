...
double domain::processDelaunayMPI(int coreNum){
	...
	for (std::set<int>::iterator it=activePartSet.begin(); it!=activePartSet.end(); ++it){
		...
		localIndexPartId++;
		if(localIndexPartId == activePartNum) break;
	}

	//take out active partId in activePartSet after add in to activePartIdArr
	for(unsigned int index=0; index<activePartNum; index++){
...
std::cout<<"triangleNum: "<<triangleNum<<"\n";
	...
	unsigned long long *tempPointIdArr = new unsigned long long[triangleNum*3];
...
//	std::string fileStr = path + "rawPointData/fullPointPart.ver";
...
void domain::drawLeftOverTriangleArr(){
	//store triangles to coordinate files (triangleCoorsAfterSecondPhase.tri)
	double *tempCoorArr = new double[triangleArrSize*6];
	for(unsigned triangleIndex=0; triangleIndex<triangleArrSize; triangleIndex++){
		tempCoorArr[triangleIndex*6] = triangleArr[triangleIndex].p1.getX();
		tempCoorArr[triangleIndex*6+1] = triangleArr[triangleIndex].p1.getY();
		tempCoorArr[triangleIndex*6+2] = triangleArr[triangleIndex].p2.getX();
		tempCoorArr[triangleIndex*6+3] = triangleArr[triangleIndex].p2.getY();
		tempCoorArr[triangleIndex*6+4] = triangleArr[triangleIndex].p3.getX();
		tempCoorArr[triangleIndex*6+5] = triangleArr[triangleIndex].p3.getY();
	}

	//store tempCoorArr to file triangleCoorsAfterSecondPhase.tri
	std::string fileStr = path + "delaunayResults/triangleCoorsAfterSecondPhase.tri";
	FILE *f = fopen(fileStr.c_str(), "wb");
	if(!f){
		std::cout<<"not exist "<<fileStr<<std::endl;
		return;
	}
	fwrite(tempCoorArr, triangleArrSize*6, sizeof(double), f);
	fclose(f);
	std::cout<<"triangleCoorsAfterSecondPhase.tri has been wrtten out to " + path + "delaunayResults/";
	delete [] tempCoorArr;

	//draw inactive triangles (left over)
	drawMesh *d = new drawMesh;
	d->	drawGridLines(xPartNum, yPartNum);
	if(triangleArr==NULL) return;
	d->drawTriangleArr(triangleArr, triangleArrSize, 3);//CYAN
	delete d;
}
//===================================================================
