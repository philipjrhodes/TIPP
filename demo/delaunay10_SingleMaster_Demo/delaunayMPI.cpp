...
//****************************************************************************
	...
	if(argc<=2){// no arguments
		...
		std::cout<<"The first argument is the domain size\n";
		std::cout<<"The second argument is the path to the dataset\n";
	}
	else{
...
//			std::cout<<pointIdArr[3*i]<<"--"<<pointIdArr[3*i+1]<<"--"<<pointIdArr[3*i+2]<<"\n";
	...
	//after we have triangle information triangleList from generateTriangles and (pointCoorArr,pointIdArr) from readPointCoordinates we will do delaunay/triangulate
	triangulate(partId);
...
void delaunayMPI::processStoreTriangles(int my_rank){
	if(storeTriangleList==NULL) return;
...
void delaunayMPI::processTriangleList(int my_rank){
...
//			std::cout<<triangleNumOffsetArr[i]<<"\n"; 
	...
	if(triangleNum!=0){
		...
		double *pointCoorArr;
		try {
			pointCoorArr = new double[triangleNum*6];
		} catch (std::bad_alloc&) {
		  // Handle error
			std::cout<<"Memory overflow!!!!!!!!!!!\n";
			exit(1);
...
void delaunayMPI::readPointCoor(int my_rank){

	if(pointNumPartSize>0){
		std::string currPath1 = path + "pointCoorPart.ver";
...
*/	
		std::string currPath2 = path + "pointIdPart.ver";
...
/*	//eighth line is startIds for points in each active partition
	startIdPartArr = new unsigned long long[partNum];
	for(int i=0; i<partNum; i++){
		triangleInfoFile >> strItem;
...
*/
	...
	try {
		tempPointIdArr = new unsigned long long[totalTriangleSize*3];
	} catch (std::bad_alloc&) {
	  // Handle error
		std::cout<<"Memory overflow!!!!!!!!!!!\n";
		exit(1);
...
//==============================================================================
void delaunayMPI::triangulate(unsigned int partId){
...
std::cout<<"start triangulation from partId = "<<partId<<std::endl;
	...
	for(unsigned int localPointIndex=0; localPointIndex<pointNumPartSize; localPointIndex++){
		...
		p.setId(pointIdArr[localPointIndex]);
/*
if(partId==13)
...
*/
	...
	delete [] pointCoorArr;
	std::cout<<"done triangulation with partId = "<<partId<<" from processor "<<processor_name<<std::endl;
}
