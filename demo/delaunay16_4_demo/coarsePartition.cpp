...
//===================================================================
coarsePartition::coarsePartition(unsigned int color, unsigned int partId, unsigned int activePartNum, unsigned long long initTriangleSize, unsigned int xPartNum, unsigned int yPartNum, std::string pathStr){
...
unsigned int coarsePartition::generateActivePartitions(){
	...
	unsigned int index;
	unsigned int unFinishSize;

	for(unsigned int i=0; i<xFinePartNum*yFinePartNum; i++) activePartArr[i] = false;

	//this piece of code for visualization only
	if(phaseCount==0){
		currActiveList.push_back(0);
		partArr[0].finish = false;
		partArr[0].active = true;

		currActiveList.push_back(3);
		partArr[3].finish = false;
		partArr[3].active = true;

		currActiveList.push_back(9);
		partArr[9].finish = false;
		partArr[9].active = true;

		currActiveList.push_back(15);
		partArr[15].finish = false;
		partArr[15].active = true;

		for(int i=0; i<xFinePartNum*yFinePartNum; i++){
			if((i!=0)&&(i!=3)&&(i!=9)&&(i!=15)){
				partArr[i].finish = false;
				partArr[i].active = false;
			}
		}
		phaseCount++;
	}
...
/*	std::string fileStr1 = generateFileName(coarsePartId, path + "delaunayResults/boundaryTriangles", xCoarsePartNum*yCoarsePartNum, ".tri");
	...
	fwrite(boundaryTrangleArr, boundaryTrangleSize, sizeof(triangle), f1);
	fclose(f1);
...
//	for(unsigned int i=0; i<currActivePartNum; i++) std::cout<<pointNumPartArr[i]<<" ";
...
//std::cout<<"totalTriangleSize: " <<totalTriangleSize<<"\n";
	...
	for(int activePartId=0; activePartId<activePartNum; activePartId++){
		...
		unsigned subActiveId = activePartIdArr[activePartId];
		unsigned subActiveTriangleNum = triangleIdList.size();
		unsigned subTriangleIndex = 0;
		double *subTriangleCoorArr = new double[subActiveTriangleNum*6];
		...
		for (std::list<unsigned long long>::iterator it=triangleIdList.begin(); it!=triangleIdList.end(); ++it){
			...
			subTriangleCoorArr[subTriangleIndex*6] = tempCoorArr[triangleIndex*6];
			subTriangleCoorArr[subTriangleIndex*6+1] = tempCoorArr[triangleIndex*6+1];
			subTriangleCoorArr[subTriangleIndex*6+2] = tempCoorArr[triangleIndex*6+2];
			subTriangleCoorArr[subTriangleIndex*6+3] = tempCoorArr[triangleIndex*6+3];
			subTriangleCoorArr[subTriangleIndex*6+4] = tempCoorArr[triangleIndex*6+4];
			subTriangleCoorArr[subTriangleIndex*6+5] = tempCoorArr[triangleIndex*6+5];
			subTriangleIndex++;

			fileStr = generateFileName(coarsePartId, path + "delaunayResults/subActiveCoorFineParts", xCoarsePartNum*yCoarsePartNum, "_");
			fileStr = generateFileName(subActiveId, fileStr, xFinePartNum*yFinePartNum, ".tri");
			FILE *f = fopen(fileStr.c_str(), "wb");
			if(!f){
				std::cout<<"not exist "<<fileStr<<std::endl;
				fclose(f);
			...
			fwrite(subTriangleCoorArr, subActiveTriangleNum*6, sizeof(double), f);
			fclose(f);

			triangleIndex++;
		}
		delete [] subTriangleCoorArr;
	}
...
//	std::string fileStr = path + "delaunayResults/tempCoorFineParts.tri";
	fileStr = generateFileName(coarsePartId, path + "delaunayResults/tempCoorFineParts", xCoarsePartNum*yCoarsePartNum, ".tri");
...
//std::cout<<"returnTriangleIdArrSize: "<<returnTriangleIdArrSize<<"\n";
...
//std::cout<<returnTriangleCoorArr[i*6]<<" "<<returnTriangleCoorArr[i*6+1]<<" "<<returnTriangleIdArr[i*3]<<" "<<returnTriangleCoorArr[i*6+2]<<" "<<returnTriangleCoorArr[i*6+3]<<" "<<returnTriangleIdArr[i*3+1]<<" "<<returnTriangleCoorArr[i*6+4]<<" "<<returnTriangleCoorArr[i*6+5]<<" "<<returnTriangleIdArr[i*3+2]<<"\n";
...
/*	//generate file interiorXX.tri for visualization
	...
	unsigned interiorTriangleCoorArrNum = storedTriangleSize;
	double *interiorTriangleCoorArr = new double[interiorTriangleCoorArrNum*6];

	index=0;
	for(std::list<triangle>::iterator it=insideTriangleList.begin(); it!=insideTriangleList.end(); it++){
		interiorTriangleCoorArr[index*6] = (*it).p1.getX();
		interiorTriangleCoorArr[index*6+1] = (*it).p1.getY();
		interiorTriangleCoorArr[index*6+2] = (*it).p2.getX();
		interiorTriangleCoorArr[index*6+3] = (*it).p2.getY();
		interiorTriangleCoorArr[index*6+4] = (*it).p3.getX();
		interiorTriangleCoorArr[index*6+5] = (*it).p3.getY();
		index++;
	}
	storeTriangleCoors(interiorTriangleCoorArr, interiorTriangleCoorArrNum, currPath, true);
	delete [] interiorTriangleCoorArr;
*/
...
//	triangle *boundaryTrangleArr = new triangle[boundaryTriangleSize];
	index = 0;
	//collect trinagleIds and triangleCoors out of boundaryTrangleArr
...
/*	//store tempTriangledArr to boundaryTrianglesXX.tri
	...
	fwrite(tempTriangledArr, boundaryTriangleSize, sizeof(triangle), f1);
	fclose(f1);

	delete [] boundaryTrangleArr;
*/

/*	//generate file boundaryXX.tri for visualization
	currPath = generateFileName(coarsePartId, path + "delaunayResults/boundary", xCoarsePartNum*yCoarsePartNum, ".tri");
	storeTriangleCoors(returnBoundaryTriangleCoorArr, boundaryTriangleSize, currPath, true);
*/
}
