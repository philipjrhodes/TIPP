...
//==============================================================================
distribute::distribute(double domainsize, unsigned int xCoarsePartNumber, unsigned int yCoarsePartNumber, unsigned int xFinePartNumber, unsigned int yFinePartNumber, std::string sourcePath, std::string verInfoFile, std::string verFile, std::string resultPath, unsigned long long chunksize, unsigned int initcoarsesize, unsigned int initfinesize){
	...
	std::cout<<"================================================\n";
	std::cout<<"xCoarsePartNumber: "<<xCoarsePartNumber<<"   yCoarsePartNumber: "<<yCoarsePartNumber<<std::endl;
	std::cout<<"xFinePartNumber: "<<xFinePartNumber<<"   yFinePartNumber: "<<yFinePartNumber<<std::endl;
	std::cout<<"================================================\n";
...
void distribute::processInitPoints(){
	...
	for(unsigned int coarsePartId=0; coarsePartId<xCoarsePartNum*yCoarsePartNum; coarsePartId++){
		...
		for(unsigned int finePartId=0; finePartId<xFinePartNum*yFinePartNum; finePartId++)
			if(initPointArr[coarsePartId][finePartId].size()<(initCoarseSize+initFineSize)){
				...
				initPartFileStr = generateFileName(coarsePartId, destPath + "initPointPart", xCoarsePartNum*yCoarsePartNum, ".ver");
	...
	std::string initDomainFileStr = destPath + "initDomainPoints.ver";
...
void distribute::createInfoFiles(){
	...
	std::ofstream pointPartInfoFile(destPath + "pointDomainInfo.xfdl", std::ofstream::out | std::ofstream::trunc);
...
//std::cout<<"reading position: "<<readingPos<<" of fileSize: "<<vertexRecordCount<<std::endl;
		std::cout<<"reading: "<<readingPos<<"/"<<vertexRecordCount<<"\n";
...
//==============================================================================
void distribute::writeListToFile(std::list<point> pointList, unsigned int partCoarseId, unsigned int partFineId){
	...
	writeToBinaryFile(destPath + "pointPart", partCoarseId, partFineId, pointArr, size);
	delete [] pointArr;
}
...
void distribute::writeToBinaryFile(std::string fileStr, unsigned int partCoarseId, unsigned int partFineId, point *data, unsigned int size){
	fileStr = generateFileName(partCoarseId, destPath + "pointPart", xCoarsePartNum*yCoarsePartNum, "");
	fileStr = generateFileName(partFineId, fileStr + "_", xFinePartNum*yFinePartNum, ".ver");
