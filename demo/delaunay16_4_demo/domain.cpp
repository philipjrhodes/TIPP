...
domain::domain(double lowX, double lowY, double highX, double highY, std::string pathStr){
	...
	std::string delCommand = "rm " + path + "delaunayResults/*.tri";
	system(delCommand.c_str());
	delCommand = "rm " + path + "delaunayResults/temp*";
	system(delCommand.c_str());
}
...
void domain::storeActivePartitions(unsigned int *activePartIdArr, unsigned int activePartNum){
	...
	for(unsigned int activePartId=0; activePartId<activePartNum; activePartId++){
		...
		//store active sub-parttitions for visualization
		unsigned activeId = activePartIdArr[activePartId];
		unsigned activeTriangleNum = triangleIdList.size();
		unsigned triangleId = 0;
		double *triangleCoorArr = new double[activeTriangleNum*6];
		...
		for (std::list<unsigned long long>::iterator it=triangleIdList.begin(); it!=triangleIdList.end(); ++it){
			...
			triangleCoorArr[triangleId*6] = tempCoorArr[triangleIndex*6];
			triangleCoorArr[triangleId*6+1] = tempCoorArr[triangleIndex*6+1];
			triangleCoorArr[triangleId*6+2] = tempCoorArr[triangleIndex*6+2];
			triangleCoorArr[triangleId*6+3] = tempCoorArr[triangleIndex*6+3];
			triangleCoorArr[triangleId*6+4] = tempCoorArr[triangleIndex*6+4];
			triangleCoorArr[triangleId*6+5] = tempCoorArr[triangleIndex*6+5];
			triangleId++;

			fileStr = generateFileName(activePartIdArr[activePartId], path + "delaunayResults/activeCoarseParts", 16, ".tri");
			...
			fwrite(triangleCoorArr, activeTriangleNum*6, sizeof(double), f);
			fclose(f);
...
void domain::addReturnStoreTriangles(){
	...
	for(unsigned int index=0; index<activePartArrSize; index++){
		fullStoreFile = generateFileName(activePartArr[index], path + "delaunayResults/triangleIds", xPartNum*yPartNum, ".tri");
		//add triangles in returnAllStoreTriangleIdsXX.tri to file triangleIds.tri, 
		addFile(fullStoreFile, path + "delaunayResults/triangleIds.tri");
...
//=========================================================================================
void domain::readTriangleIdArr(unsigned long long *&triangleIdArr, unsigned long long &triangleNum, std::string fileStr){
	...
	triangleNum = ftell(f)/(sizeof(unsigned long long)*3); // get current file pointer
...
//=========================================================================================
void domain::readTriangleCoorArr(double *&triangleCoorArr, unsigned long long &triangleNum, std::string fileStr){
	FILE *f = fopen(fileStr.c_str(), "rb");
	if(f==NULL){
		std::cout<<"not exist "<<fileStr<<std::endl;
		exit(1);
	...
	fseek(f, 0, SEEK_END); // seek to end of file
	triangleNum = ftell(f)/(sizeof(double)*6); // get current file pointer
	fseek(f, 0, SEEK_SET); // seek back to beginning of file
...
//std::cout<<"undelivered triangles: "<<count<<"\n";
	...
	std::string fileStr1 = path + "delaunayResults/boundaryIds.tri";
	...
	readTriangleIdArr(returnBoundaryTriangleIdArr, boundaryTriangleNum, fileStr1);
	readTriangleCoorArr(returnBoundaryTriangleCoorArr, boundaryTriangleNum, fileStr2);
	removeFile(fileStr1);
	removeFile(fileStr2);
...
void domain::addReturnTriangles(){
	...
	for(unsigned int coarsePartId=0; coarsePartId<xPartNum*yPartNum; coarsePartId++){
		fullStoreFile = generateFileName(coarsePartId, path + "delaunayResults/returnAllStoreTriangleIds", xPartNum*yPartNum, ".tri");
		//add triangles in returnAllStoreTriangleIdsXX.tri to file triangleIds.tri, 
		std::cout<<"Adding " + fullStoreFile + " to triangleIds.tri\n";
		addFile(fullStoreFile, path + "delaunayResults/triangleIds.tri");
		removeFile(fullStoreFile);
...
void domain::readFinePartitionSize(unsigned int &xFinePartNum, unsigned int &yFinePartNum){
	//Read information from pointDomainInfo.xfdl
	std::string fileInfoStr = generateFileName(0, path + "delaunayResults/pointPartInfo", xPartNum*yPartNum, ".xfdl");
	std::ifstream vertexPartInfoFile(fileInfoStr.c_str());
	if(!vertexPartInfoFile){
		std::cout<<"non exist "<<fileInfoStr.c_str()<<std::endl;
		exit(1);
